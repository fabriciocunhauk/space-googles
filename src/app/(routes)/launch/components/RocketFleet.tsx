...
function RocketCard({ rocket }: { rocket: RocketSpec }) {
  return (
    <div className="glass rounded-[32px] overflow-hidden border border-white/10 group hover:border-white/30 transition-all flex flex-col h-full">
      <div className="h-48 overflow-hidden relative">
        ...
        {!rocket.active && (
          <div className="absolute top-4 right-4 bg-white/10 backdrop-blur-md px-3 py-1 rounded-full border border-white/20">
            <span className="text-[10px] text-nebula-blue/60 uppercase tracking-widest font-Barlow-Condensed">
              Legacy
            </span>
          </div>
        )}
      ...
      <div className="p-8 flex flex-col flex-grow space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-nebula-blue text-xs font-Barlow opacity-70 leading-relaxed line-clamp-3">
            {rocket.description}
          </p>
        </div>
        ...
        <div className="space-y-3 pt-4 border-t border-white/5 mt-auto">
          {[
            ...
            <div
              key={j}
              className="flex justify-between items-center text-[10px] uppercase font-Barlow-Condensed tracking-[2px]"
            >
    ...
    <div className="space-y-12 pt-16 border-t border-white/10">
      <div className="text-center space-y-4">
        ...
        <p className="text-nebula-blue font-Barlow max-w-2xl mx-auto opacity-70">
          Explore the specifications of the world&apos;s most advanced
          spacecraft, dynamically updated from SpaceX telemetry.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {rockets.map((rocket, i) => (
          <RocketCard key={i} rocket={rocket} />
        ))}
      </div>
    </div>
  );
